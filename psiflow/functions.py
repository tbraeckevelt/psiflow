...
class ExtendedHarmonicFunction(EnergyFunction):
    ...
    def __post_init__(self):
        ...
        self._TcTsinv = self._Tc @ np.linalg.inv(self._Ts)
    ...
    def __call__(
        ...
        Trs = Tr @ self._Ts
        trans_pos = geometry.per_atom.positions @ Tr @ self._Ts
        Tgradc = self._TcTsinv @ np.linalg.pinv(trans_pos)

        diff_pos  = trans_pos - self.positions @ Tr @ self._Ts
        ...
        if self.energy is not None:
            energy += self.energy
        grad_pos = grad_T[:-9].reshape(-1, 3) @ Trs + Tgradc.T @ grad_T[-9:].reshape(3, 3)  # TO DO: check if this is correct
        grad_cell = self._Tc.T @ grad_T[-9:].reshape(3, 3)                  # TO DO: check if this is correct
        forces = (-1.0) * grad_pos
        volume = np.linalg.det(geometry.cell)
        stress = (1/volume) * geometry.cell.T @ grad_cell                   # TO DO: check if this is correct
